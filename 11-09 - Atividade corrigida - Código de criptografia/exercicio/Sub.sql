...
FROM Imovel 
WHERE CdBairro = (SELECT CdBairro FROM Imovel WHERE CdImovel = 2)
  AND CdImovel <> 2;

--2 - Lista de todos os imóveis que custam mais que a média de preço dos imóveis:

SELECT * 
FROM Imovel 
WHERE VlPreco > (SELECT AVG(VlPreco) FROM Imovel);

--3 - Lista de todos os compradores que tenham ofertas cadastradas com valor superior a 70 mil:
...
--4 - Lista com todos os imóveis com oferta superior à média do valor das ofertas:
...
--5 - Lista com todos os imóveis com preço superior à média de preço dos imóveis do mesmo bairro:

SELECT * 
FROM Imovel
WHERE VlPreco > (
    SELECT AVG(VlPreco) 
    FROM Imovel WHERE Imovel.CdBairro = Imovel.CdBairro
);

--6 - Lista com os imóveis que têm o preço igual ao menor preço de cada vendedor:

SELECT * 
FROM Imovel
WHERE VlPreco = (
    SELECT MIN(VlPreco) 
    FROM Imovel WHERE Imovel.CdVendedor = Imovel.CdVendedor
);

--7 - Lista com os imóveis que têm o preço igual ao menor de todos os vendedores, exceto os imóveis do próprio vendedor:
...
--8 - Lista com as ofertas menores que todas as ofertas do comprador 2, exceto as ofertas do próprio comprador:

SELECT * 
FROM Oferta
WHERE VlOferta < (
    SELECT MIN(VlOferta) 
    FROM Oferta  WHERE Oferta.CdComprador = 2
) 
AND CdComprador <> 2;
