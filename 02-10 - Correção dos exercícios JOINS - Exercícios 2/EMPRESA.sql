...
SELECT DISTINCT DATENAME(MONTH, DataNasc) AS Mes
FROM Funcionarios
ORDER BY Mes;
...
Select C.Descr, P.Descr
from Categorias C CROSS JOIN Produtos P

--1 - Mostre todos os dados dos clientes que fizeram pedidos em 1996.
SELECT Clientes.*
FROM Clientes
INNER JOIN Pedidos ON Clientes.CodCli = Pedidos.CodCli
WHERE YEAR(Pedidos.DataPed) = 1996;

-- 2 Apresente todos os nomes dos funcionários que fizeram pedidos para o cliente “Around the horn”:
SELECT Funcionarios.Nome
FROM Funcionarios
INNER JOIN Pedidos ON Funcionarios.CodFun = Pedidos.CodFun
INNER JOIN Clientes ON Pedidos.CodCli = Clientes.CodCli
WHERE Clientes.Nome = 'Around the horn';

-- 3 Mostre todos os dados dos pedidos feitos pelo cliente “Comércio Mineiro”:
SELECT Pedidos.*
FROM Pedidos
INNER JOIN Clientes ON Pedidos.CodCli = Clientes.CodCli
WHERE Clientes.Nome = 'Com?rcio Mineiro';

-- 4 Exiba todos os dados dos funcionários que participaram dos pedidos de setembro de 1996:
SELECT Funcionarios.*
FROM Funcionarios
INNER JOIN Pedidos ON Funcionarios.CodFun = Pedidos.CodFun
WHERE MONTH(Pedidos.DataPed) = 9 AND YEAR(Pedidos.DataPed) = 1996;

-- 5 Apresente todos os dados e as categorias de todos os produtos da categoria “laticínios”:
SELECT Produtos.*, Categorias.Descr
FROM Produtos
INNER JOIN Categorias ON Produtos.CodCategoria = Categorias.CodCategoria
WHERE Categorias.Descr = 'latic?nios';

-- 6 Exiba todos os dados dos produtos e o número do pedido de que eles fizeram parte da data “1996-07-08”:
SELECT Produtos.*, Pedidos.NumPed
FROM Produtos
INNER JOIN DetalhesPed ON Produtos.CodProd = DetalhesPed.CodProd
INNER JOIN Pedidos ON DetalhesPed.NumPed = Pedidos.NumPed
WHERE Pedidos.DataPed = '1996-07-08';

-- 7 Mostre o nome de todos os funcionários e o número dos pedidos que eles fizeram em “1997-05-01”:
SELECT Funcionarios.Nome, Funcionarios.Sobrenome, Pedidos.NumPed
FROM Funcionarios
INNER JOIN Pedidos ON Funcionarios.CodFun = Pedidos.CodFun
WHERE Pedidos.DataPed = '1997-05-01';

--8 Mostre o nome do funcionário e todos os dados dos pedidos feitos pelos funcionários com salários maiores que 10000,00:
SELECT Funcionarios.Nome, Funcionarios.Sobrenome, Pedidos.*
FROM Funcionarios
INNER JOIN Pedidos ON Funcionarios.CodFun = Pedidos.CodFun
WHERE Funcionarios.Salario > 10000;
...
FROM Pedidos
INNER JOIN Clientes ON Pedidos.CodCli = Clientes.CodCli
WHERE MONTH(Pedidos.DataPed) = 5 AND YEAR(Pedidos.DataPed) = 1997;
...
SELECT DISTINCT Categorias.Descr, Produtos.Descr
FROM Categorias
INNER JOIN Produtos ON Categorias.CodCategoria = Produtos.CodCategoria
INNER JOIN DetalhesPed ON Produtos.CodProd = DetalhesPed.CodProd
INNER JOIN Pedidos ON DetalhesPed.NumPed = Pedidos.NumPed
WHERE DetalhesPed.Qtde <= 10 AND YEAR(Pedidos.DataPed) = 1998
ORDER BY Categorias.Descr DESC, Produtos.Descr DESC;
